...
class SoraClient:
    ...
    def _handle_error_response(self, response: httpx.Response) -> None:
        ...
        if status in (401, 403):
            raise SoraAuthError(message)
        raise SoraAPIError(message=message, code=code, status_code=status)
    ...
    async def generate_video(self, **kwargs: Any) -> dict[str, Any]:
        ...
        return await self.request("/sora/videos", self._with_async_callback(kwargs))
