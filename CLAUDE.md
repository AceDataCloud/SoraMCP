...
MCP (Model Context Protocol) server for Sora (OpenAI) AI video generation via AceDataCloud API.
...
core/
  ...
  types.py      — Literal types (SoraModel, etc.)
...
1. **Source of truth** — `_docs/openapi/sora.json` is the OpenAPI spec for the SoraMCP API.
